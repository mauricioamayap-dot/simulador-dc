...
EQ_NUM = {'L1 — SECO':1,'L2 — SECO':2,'L3 — SECO':3,
           'DASA — FRÍO':4,'Facturación':5,'Inventarios':6,'Devolutivos':7,'QDV':8}
...
def parse_upload(file_bytes):
    ...
    try:
        if HAS_OPENPYXL:
            try:
                ...
                # Prefer "Personal" sheet, otherwise first sheet with data
                ws = None
                for sh_name in wb.sheetnames:
                    if 'personal' in sh_name.lower():
                        ws = wb[sh_name]; break
                if ws is None:
                    ...
                    for sh_name in wb.sheetnames:
                        sh = wb[sh_name]
                        n = sum(1 for row in sh.iter_rows(values_only=True) if any(c for c in row if c))
                        if n > best[1]: best = (sh, n)
                    ws = best[0] or wb.active
                rows=list(ws.iter_rows(values_only=True))
            except Exception:
                # Try as CSV with multiple encodings
                for enc in ['utf-8','latin-1','cp1252','iso-8859-1']:
                    ...
                    return None,["No se pudo leer el archivo. Guárdalo como .xlsx o .csv UTF-8"]
        else:
            import csv
            for enc in ['utf-8','latin-1','cp1252','iso-8859-1']:
                try:
                    rows=list(csv.reader(io.StringIO(file_bytes.decode(enc))))
                    break
                except:
                    continue
            else:
                return None,["Error de codificación. Guarda el CSV como UTF-8"]
    ...
    for i,row in enumerate(rows[:10]):
        ...
        if any(v in ['nombre','name'] for v in vals) and any(v in ['rol','role'] for v in vals):
            header_row=i; break
    if header_row is None:
        # Try to find row with N°/Nombre/Rol in any form
        for i,row in enumerate(rows[:10]):
            if not row: continue
            vals=[str(c).strip().lower() for c in row if c]
            if 'nombre' in ' '.join(vals) or 'name' in ' '.join(vals):
                header_row=i; break
    if header_row is None:
        return None,["No se encontró encabezado. La hoja 'Personal' debe tener columnas: N°, Nombre, Rol, EQUIPO"]
    ...
    for r_idx,row in enumerate(rows[header_row+1:],1):
        ...
        try:
            ...
            if not nom: continue
            # Skip section header rows (merges like "EQUIPO 1 — L1 SECO")
            if 'equipo' in nom.lower() and '—' in nom: continue
            if nom.startswith('  EQUIPO'): continue
...
'QDV':{'min_oper':2,'n':3,'people':[
    (1,'POR CONTRATAR','Alistador','DC1',J(1)),
    (2,'POR CONTRATAR','Alistador','DC1',J(2)),
    (3,'POR CONTRATAR','Alistador','DC1',J(3)),
]}}
